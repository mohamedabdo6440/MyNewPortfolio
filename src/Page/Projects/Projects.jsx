...
const Projects = memo(() => {
    ...
    // console.log(data.map(ele => ele.image));
    useEffect(() => {
        if (projects.length) {
            setData(projects)
        }
    ...
    return (
        <motion.section
            ...
            <motion.h2 initial={{ opacity: 0 }} viewport={{ once: true }} whileInView={{ opacity: 1 }} transition={{ duration: .6, delay: .2 }} className='pt-12 containerEdit font-bold text-center text-[#04c2c9] text-3xl'>Some of my current projects are on 'Github'</motion.h2>
